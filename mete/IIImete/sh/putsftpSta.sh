...
#dsc          :
#    Get the status of uploading sftp server
#
#invoke eg    :
#   ./putsftpSta.sh 0 0 192.168.0.51 fczx fczx "/home/fczx/fushikai_test/sftp_dir/up" "/home/fczx/tmp/up" "busilist_20201010_0.xml"
#
#version chg  :
#   20201120@created
#
...
fileName=$6 #file name to be uploaded to the sftp server
...
sftpRet=$(getSftpSerStatu "${opFlag}" "${trsType}" "${trsMode}" "${sftpIP}" "${sftpUser}" "${sftpPwd}" "${sftpRdir}" "${sftpLdir}" "${fileName}" "${sftpCtrPNum}")
ret=$?
logTime="$(date '+%Y/%m/%d %H:%M:%S.%N')"

#local path error
if [ ${ret} -eq 10 ];then
    writeLog 2 "${logTime}the local file path [${sftpLdir}] is error\n"
    exit 5
fi

if [ ${ret} -eq 11 ];then
    writeLog 2 "${logTime} connet sftp server [${sftpIP} ${sftpCtrPNum}] is error\n"
    exit 1
fi
...
if [ ${ret} -eq 12 ];then
    writeLog 2 "\n${logTime} sftpRet=[${sftpRet}]"
    writeLog 2 "${logTime} the sftp username [${sftpUser}] or passwd [${sftpPwd}] is error\n"
    exit 2
fi
...
if [ ${ret} -eq 13 ];then
    writeLog 2 "\n${logTime} sftpRet=[${sftpRet}]"
    writeLog 2 "${logTime} the file path [${sftpRdir}]is error\n"
    exit 3
fi
...
writeLog 4 "\n${logTime} sftpRet=[${sftpRet}]"
writeLog 1 "${logTime} The sftp server [${sftpIP} ${sftpCtrPNum}] is in a normal state,and the file [${fileName}] exists in the local [${sftpLdir}] path!\n"
