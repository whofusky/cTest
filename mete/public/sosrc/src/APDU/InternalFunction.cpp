             ...
             )
{
    ...
    for ( u_char i = 0; i < bodyObjNum; i++ )
    {
        ...
        {
            bodySet_p = bodySet_p + ( i * bodySetLng );
            tmpRet = apdu_p->setAPDU_ELEMENT ( bodySet_p, bodySetLng,
                                               BODY_SET_INDEX,
                                               i );
        ...
        {
            tsp_p = ( ( u_char* ) tsp_p ) + ( i * tOneTspLng );
            tmpRet = apdu_p->setBODY_TSP_VAL ( tsp_p, i );
                ...
                )
{
    ...
    */
    ...
    if ( ( u_char ) funDsc_p->act_prio_chnlID == PASSIVE_TYPE )
    {
        //被动站验收主动站的数据
    ...
    else if ( ( u_char ) funDsc_p->act_prio_chnlID == ACTIVE_TYPE )
    {
        //主动站验收被动站的数据;
                   ...
                   )
{
    ...
    if ( tmpTID >= MC_TID_136 && tmpTID <= MC_TID_140 )
    {
        ...
        if ( act_prio_chnlID == ACTIVE_TYPE ) //主动
        {
            ...
            else
            {
                if ( nowFrameNum_resp < allFrameNum_resp - 1 )
                {
                    ...
                    *nxtStepStat_p = MC_SEND_CNUE_EXEC;
                          ...
                          || nowFrameNum_resp == allFrameNum_resp
                        )
                {
                    //已经是限定值了不需要发送帧了
                    if ( nowFrameNum_req < allFrameNum_req )
                  ...
                  )
{
    ...
    switch ( funDsc_p->ssnDscNo )
    {
        ...
        case CLK_SYN: //时钟同步
        {
            if ( act_prio_chnlID == PASSIVE_TYPE ) //被控站的验收主控站的数据
            {
                if ( allFrameNum_req != 1 )
                {
                    return ERR_FRAME_NO;
            ...
            else if ( act_prio_chnlID == ACTIVE_TYPE ) //主控站验收被控站的数据
            {
                if ( allFrameNum_resp != 1 )
                {
                    return ERR_FRAME_NO;
        ...
        case ACMU_FRZ:
        {
            /*累积量传输前的冻结
              ...
              控制站先冻结再请求传输分离出来的
             */
            if ( act_prio_chnlID == PASSIVE_TYPE ) //被动站接收的是主动站的数据
            {
                if ( tmpCOT != MC_COT_6
                     || tmpCOT != tRemoteSet_p->COT
                   )
            ...
            else if ( act_prio_chnlID ==
                      ACTIVE_TYPE ) //主站接收的是被动站的数据
            {
                if ( tmpCOT != MC_COT_7
                     || tmpCOT != tRemoteSet_p->COT
                   )
        ...
        case ACMU_TRNS: //累积量传输
        {
            switch ( usingMode )
            {
                case CTLD_DRCTSONPT_TRNS://模式0，被控站直接突发传输
                {
                    ...
                    if ( act_prio_chnlID == PASSIVE_TYPE ) //被动站
                    {
                        if ( allFrameNum_req != 0 )
                        {
                            return ERR_FRAME_NO;
                    ...
                    else if ( act_prio_chnlID == ACTIVE_TYPE )  //控制站
                    {
                        if ( allFrameNum_resp != 1 )
                        {
                            return ERR_FRAME_NO;
        ...
        case TST_PRCD: //测试过程
        {
            if ( act_prio_chnlID == PASSIVE_TYPE ) //被控站接收的是控制站的数据
            {
                if ( tmpCOT != MC_COT_6
                     || tmpCOT != tRemoteSet_p->COT
                   )   //传送原因
