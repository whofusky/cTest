...
//   p1:预测主  p2:预测备  s: scada    m: meteorological气象
typedef enum{
    ...
    PV_DIR_APPSENDM,     // app-send-m  用于气象服务器IMS接收场外指令包等
    ...
    PV_DIR_WARFORIMS,    // temp-warn    各软件生成的报警文件(供IMS外传用)
...
typedef enum{
    ...
    RUNPAR_HOSTNAME,// 程序当前主机名:取的FSHOSTNAME环境变量配置值
    RUNPAR_COUNT
}APP_RUNPAR_TYPE;
