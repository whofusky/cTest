...
//u_short stnAcsFlag 的位存储变量
typedef struct{
    u_short  ctlType:2;     //位0 = 1存储子系统执行数据值的保存；位1 = 1存储子系统执行数据值的加载；全 = 0, do nothing；全 = 1，加载且保存
    u_short  ctlRes:1;      //保留位
    u_short  dataValid:1;    //ADU元素第3位PDU_DATA_INDEX ，=1表示ctlType的操作在此元素中有效，=0为无效
    u_short pad:12;
}T_stnAcsFlagM;
...
typedef struct
{
    ...
    //add@20180625
    char      chnlRole;//通道（站）角色
    //add@20180626
    char      frmDirect;//数据帧的方向：控制或监视
} T_ptclFunDsc;
...
/*
    20180223133916 注释 (*reqEleVal_pp)[11] 及 (*respEleVal_pp)[11]
                   添加 reqData 及 respData
*/
typedef struct
{
    /*位0 = 1存储子系统执行数据值的保存；位1 = 1存储子系统执行数据值的加载；全 = 0,
    ...
    */
    ...
    //请求帧个数
    u_char       reqAPDUNum;
    u_char*      reqBody_setNum_p;//每一帧信息体对象中点的个数
    ...
    //响应帧个数
    u_char       respAPDUNum;
    u_char*      respBody_setNum_p;//每一帧信息体对象中点的个数
...
typedef struct
{
    ...
    short       kValue;
    u_char      stage;
    //T_mbsChkNode的空间记录请求帧中的功能码和事物处理标识，以便核对服务端发回的响应帧
