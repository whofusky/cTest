                       ...
                       )
{
    ...
    if ( stnSnnPara_p->actFlag == ACTIVE_TYPE )
    {
            ...
            ) //遍历所有请求帧
        {
            ...
            //根据可变结构限定词取信息体对象个数
            if( tmpVsqVal_p->SQ == 1 )//单对象结构
            {
                infBdNum = 1;
                //此时tmpVsqVal_p->N也表示点的个数
                if( tStepCfg_p->reqBody_setNum_p[i] != tmpVsqVal_p->N )
                {
                    return ERR_CONFIG;
                }
            }
            else if ( tmpVsqVal_p->SQ == 0 )//多对象结构
            {
                infBdNum = tmpVsqVal_p->N;
                //多对象时，只支持一个对象只包含一个点
                if( tStepCfg_p->reqBody_setNum_p[i] != 1 )
                {
                    return ERR_CONFIG;
                }
            }
            ...
            //tStepCfg_p->reqBody_setNum_p[i] 信息对象中点的个数
            if ( infBdNum > 0
                 && bodyObj_p == NULL
               )
            {
                ...
                )
            {
                ...
                //取点值
                for ( u_char k = 0;
                      k < tStepCfg_p->reqBody_setNum_p[i];
                      k++
                    )
                {
                ...
                if ( tStepCfg_p->reqBody_setNum_p[i] == 0 ) //信息体没有元素集
                {
    ...
    else //PASSIVE_TYPE
    {
            ...
            ) //遍历所有响应帧
        {
            ...
            if( tmpVsqVal_p->SQ == 1 )//单对象结构
            {
                infBdNum = 1;
                //此时tmpVsqVal_p->N也表示点的个数
                if( tStepCfg_p->respBody_setNum_p[i] != tmpVsqVal_p->N )
                {
                    return ERR_CONFIG;
                }
            }
            else if ( tmpVsqVal_p->SQ == 0 )//多对象结构
            {
                infBdNum = tmpVsqVal_p->N;
                //多对象时，只支持一个对象只包含一个点
                if( tStepCfg_p->respBody_setNum_p[i] != 1 )
                {
                    return ERR_CONFIG;
                }
            }
            ...
            //tStepCfg_p->respBody_setNum_p[i] 信息对象中点的个数
            if ( infBdNum > 0
                 && bodyObj_p == NULL
               )
            {
                ...
                )
            {
                ...
                //取点值
                for ( u_char k = 0;
                      k < tStepCfg_p->respBody_setNum_p[i];
                      k++
                    )
                {
                    tUsrData_p->gData.pntType = 0;
                ...
                if ( tStepCfg_p->respBody_setNum_p[i]== 0 ) //信息体没有元素集
                {
                    tUsrData_p->gData.pntType = PNTTYPE_NULL_SET;
                    stnSnnPara_p->pntNum++;
                    setNum ++;
                     ...
                     )
{
    ...
    switch ( ssnNo ) //判断功能;
    {
        ...
        case QUE_DATA://查询数据;
        {
            ...
            tmpFuncDsc.chnlRole = MASTER_STATION;
            tmpFuncDsc.frmDirect       = MC_CONTROL_DIRECTION;
        ...
        case CYC_TRNS://周期传输;
        {
            ...
            tmpFuncDsc.act_prio_chnlID  = ACTIVE_TYPE;
            tmpFuncDsc.chnlRole = SLAVE_STATION;
            tmpFuncDsc.frmDirect        = MC_MONITOR_DIRECTION;
            tmpRet = asgmtNxtFrmPar ( &tmpNowRlStep,
                                      preStepRul_p,
                                      &nxtFrmStepNo,
                                      &tmpFuncDsc,
                   ...
                   )
{
    ...
    if ( funDsc_p->chnlRole != SLAVE_STATION
        && funDsc_p->chnlRole != MASTER_STATION
        )
    {
        return ERR_CONFIG;
    }
...
/******************************************************************************
  ...
  *     协议编号：如101、104
  *修改历史
  *    20180626 修改此接口把function[*funNum_p].act_prio_chnlID的含义理解成当前站
  *             会话的主被动性（此主被动性含义举例：如站召唤在控制站为主动，则在被
  *                控站则为被动）有些会话要判断主被动性需要结合站的角色如：突发会话
  *             function[*funNum_p].frmDirect理解成数据帧的方向
*******************************************************************************/
                     ...
                     )
{
    ...
    if ( funDsc_p[0].chnlRole != MASTER_STATION
        && funDsc_p[0].chnlRole != SLAVE_STATION
       )
    {
        return ERR_CONFIG;
    }
    ...
    *funNum_p = 0;
    ...
    if ( TID >= MC_TID_136 && TID <= MC_TID_140 )
    {
        switch ( TID )
        {
            case MC_TID_136://STARTDT
                ...
                if ( COT == MC_COT_6 )
                {
                    function[*funNum_p].frmDirect = MC_CONTROL_DIRECTION;
                    function[*funNum_p].act_prio_chnlID = PASSIVE_TYPE;
                }
                else if ( COT == MC_COT_7 )
                {
                    function[*funNum_p].frmDirect = MC_MONITOR_DIRECTION;
         ...
         && TID <= MC_TID_29 ) //22...29 为将来兼容定义保留
    {
        ...
        function[0].frmDirect = MC_MONITOR_DIRECTION;
        function[0].act_prio_chnlID = ACTIVE_TYPE;

        function[1].ssnDscNo        = CYC_TRNS;
        function[1].frmDirect = MC_MONITOR_DIRECTION;
        if ( funDsc_p[0].chnlRole == MASTER_STATION )
        {
            function[1].act_prio_chnlID   = PASSIVE_TYPE;
        }
        else
        {
            function[1].act_prio_chnlID   = ACTIVE_TYPE;
        }

        function[2].ssnDscNo          = ACQ_EVT;
        // function[2].act_prio_chnlID = ACTIVE_TYPE;
        function[2].frmDirect   = MC_OUTBREAK_DIRECTION;
        if ( funDsc_p[0].chnlRole == MASTER_STATION )
        {
            function[2].act_prio_chnlID   = PASSIVE_TYPE;
        }
        else
        {
            function[2].act_prio_chnlID   = ACTIVE_TYPE;
        ...
        function[3].frmDirect = MC_MONITOR_DIRECTION;
        function[3].act_prio_chnlID = ACTIVE_TYPE;

        function[4].ssnDscNo        = CMD_TRNS;
        function[4].frmDirect = MC_MONITOR_DIRECTION;
        function[4].act_prio_chnlID = ACTIVE_TYPE;

        function[5].ssnDscNo        = ACMU_TRNS;
        function[5].frmDirect = MC_MONITOR_DIRECTION;
        function[5].act_prio_chnlID = ACTIVE_TYPE;

        function[6].ssnDscNo        = PARA_LD;
        function[6].frmDirect = MC_MONITOR_DIRECTION;
        function[6].act_prio_chnlID = ACTIVE_TYPE;

        function[7].ssnDscNo        = TST_PRCD;
        function[7].frmDirect = MC_MONITOR_DIRECTION;
        function[7].act_prio_chnlID = ACTIVE_TYPE;
       ...
       )
    {
        ...
        //function[*funNum_p].act_prio_chnlID = ACTIVE_TYPE;
        function[*funNum_p].frmDirect = MC_OUTBREAK_DIRECTION;
        if ( funDsc_p[0].chnlRole == MASTER_STATION )
        {
            function[*funNum_p].act_prio_chnlID   = PASSIVE_TYPE;
        }
                  ...
                  )
{
    ...
    //通道（站）的角色：控制站（主站）、被控制站（子站）
    if ( funDsc_p->chnlRole != MASTER_STATION
        && funDsc_p->chnlRole != SLAVE_STATION
        )
    {
        return ERR_CONFIG;
    }
                     ...
                     )
{
    ...
    if ( funDsc_p->frmDirect != MC_CONTROL_DIRECTION 
        && funDsc_p->frmDirect != MC_MONITOR_DIRECTION
        )
    {
        return ERR_CONFIG;
    }
    ...
    {
    	if( funDsc_p->frmDirect == MC_CONTROL_DIRECTION)//控制方向
    			{
    	    		apdu104_p = (APDU104*)stepRl_p->respAPDU_Obj_p;
    ...
    else //被控站
    {
        if( funDsc_p->frmDirect == MC_CONTROL_DIRECTION)//控制方向
        {
            if (act_prio_chnlID == PASSIVE_TYPE )//是请求帧
            {
                apdu104_p = (APDU104*)stepRl_p->reqAPDU_Obj_p;	
            }
            else
